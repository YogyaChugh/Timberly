...
window.Fetch.POST = function* POST(url, data) {
    ...
    let done = false;
    let content = null;   // reset each call
    ...
    while (!done) {
        yield;   // let pygbag spin
    }

    let result = content;
    content = null;   // 🧹 clear before exit
    yield result;     // ✅ one-shot value
};
...
window.Fetch.GET = function* GET(url) {
    ...
    let done = false;
    let content = undefined;  // always reset
    ...
    async def post(self, url, data=None):
        ...
        if self.is_emscripten:
            from urllib.parse import urlencode
            form_data = urlencode(data)   # ✅ encode dict to query string
            await asyncio.sleep(0)
            content = await platform.jsiter(platform.window.Fetch.POST(url, form_data))
            if self.debug:
                self.print(content)
            self.result = content
        else:
            ...
            self.result = self.requests.post(
                url,
                data=data,  # requests auto form-encodes
