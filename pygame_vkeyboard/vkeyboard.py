        ...
        if self.state == 1:
            ...
            for event in events:
                if event.type == pygame.FINGERDOWN:
                    if event.finger_id not in self.down_fingers:
                        self.down_fingers.append(event.finger_id)
                        display_size = pygame.display.get_surface().get_size()
                        finger_pos = (event.x * display_size[0], event.y * display_size[1])
                        key = self.layout.get_key_at(finger_pos)
                        if key and key.value not in listofkeys:
                    ...
                    ignore_mouse = False
                    try:
                        self.down_fingers.remove(event.finger_id)
                    except:
                        pass
                elif event.type == pygame.MOUSEBUTTONDOWN\
                        and event.button in (1, 2, 3) and not self.ignore_mouse:
                    ...
                    if key:
                        ...
                        self.layout.selection = None
                        self.input.set_selected(1)
                elif event.type == pygame.KEYDOWN and not self.ignore_mouse:
                ...
                elif event.type == pygame.JOYHATMOTION and not self.ignore_mouse:
                    if event.value == JOYHAT_LEFT and\
                        ...
                        and event.button == 0 and self.layout.selection and not self.ignore_mouse:
